...
public class SwingViewFactory implements ViewFactory {
  ...
  public View createView3D(Home home, UserPreferences preferences, HomeController3D homeController3D) {
    try {
      if (!Boolean.getBoolean("com.eteks.sweethome3d.no3D")) {
        System.out
            .println("SwingViewFactory View createView3D(Home home, UserPreferences preferences,HomeController3D homeController3D)");
        return new HomeComponent3D(home, preferences, homeController3D);
      }
    } catch (AccessControlException ex) {
      // If com.eteks.sweethome3d.no3D property can't be read,
      // security manager won't allow to access to Java 3D DLLs required by
      // HomeComponent3D class too
    }
    return null;
  }
  ...
  public DialogView createCaseView(UserPreferences preferences, HomeApplication application,
                                   HomeController homeController) {
    return new CasePanel(preferences, application, homeController);
  }
  ...
  /**
   * Returns a new view that displays the different steps that helps user to
