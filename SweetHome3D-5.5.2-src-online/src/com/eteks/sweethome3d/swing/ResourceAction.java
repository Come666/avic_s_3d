...
public class ResourceAction extends AbstractAction {
   ...
   * @param preferences user preferences used to retrieve localized properties
   *          of the action
   * @param resourceClass the class used as a context to retrieve localized
   *          properties of the action
   * @param actionPrefix prefix used in resource bundle to search action
   *          properties
   */
  ...
  /**
   * Creates an action with properties retrieved from a resource bundle in which
   * key starts with <code>actionPrefix</code>.
   * @param preferences user preferences used to retrieve localized description
   *          of the action
   * @param resourceClass the class used as a context to retrieve localized
   *          properties of the action
   * @param actionPrefix prefix used in resource bundle to search action
   *          properties
   * @param enabled <code>true</code> if the action should be enabled at
   *          creation.
   */
  public ResourceAction(UserPreferences preferences, Class<?> resourceClass, String actionPrefix, boolean enabled) {
    System.out.println("resourceClass" + resourceClass + "  actionPrefix:" + actionPrefix);
    ...
    preferences.addPropertyChangeListener(UserPreferences.Property.LANGUAGE, new LanguageChangeListener(this,
        resourceClass, actionPrefix));
  }

  /**
   * Preferences property listener bound to this action with a weak reference to
   * avoid strong link between preferences and this action.
   */
  private static class LanguageChangeListener implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent ev) {
      ...
      if (resourceAction == null) {
        ...
        resourceAction.readActionProperties((UserPreferences)ev.getSource(), this.resourceClass, this.actionPrefix);
      }
    }
  }
  ...
  private void readActionProperties(UserPreferences preferences, Class<?> resourceClass, String actionPrefix) {
    ...
    putValue(LONG_DESCRIPTION, getOptionalString(preferences, resourceClass, propertyPrefix + LONG_DESCRIPTION, false));

    String smallIcon = getOptionalString(preferences, resourceClass, propertyPrefix + SMALL_ICON, false);
    System.out.println(" smallIcon:" + smallIcon+" resourceClass:"+resourceClass+ "  propertyPrefix + SMALL_ICON:"+(propertyPrefix + SMALL_ICON));
    if (smallIcon != null) {
      ImageIcon icon = null;
      try {
        icon = SwingTools.getScaledImageIcon(resourceClass.getResource(smallIcon));
      } catch (Exception e) {
        e.printStackTrace();
      }
      // putValue(SMALL_ICON,
      // SwingTools.getScaledImageIcon(resourceClass.getResource(smallIcon)));
    ...
    int indexer = random.nextInt(100);
    for (Object key : this.getKeys()) {
      System.out.println(indexer + " readActionProperties " + key.toString() + ":" + this.getValue(key.toString()));
    }
  ...
  private static class AbstractDecoratedAction implements Action {
      ...
      action.addPropertyChangeListener(new PropertyChangeListener() {
        public void propertyChange(PropertyChangeEvent ev) {
          ...
          if ("enabled".equals(propertyName)) {
            ...
            // value is null !)
            if (newValue != null) {
              propertyChangeSupport.firePropertyChange(new PropertyChangeEvent(ev.getSource(), propertyName, ev
    ...
    public final void putValue(String key, Object value) {
      System.out.println(String.format("resourceAction putValue:[key %s: value %s]", key, value));
