...
public class HomeFurniturePanel extends JPanel implements DialogView {
  ...
  private NullableCheckBox              scanCheckBox;
  ...
  private void createComponents(UserPreferences preferences, final HomeFurnitureController controller) {
    ...
    if (controller.isPropertyEditable(HomeFurnitureController.Property.NAME_VISIBLE)) {
      ...
      this.nameVisibleCheckBox.addChangeListener(new ChangeListener() {
        public void stateChanged(ChangeEvent ev) {
          ...
          controller
              .addPropertyChangeListener(HomeFurnitureController.Property.NAME_VISIBLE, nameVisibleChangeListener);
        }
      });
    }

    if (controller.isPropertyEditable(HomeFurnitureController.Property.DESCRIPTION)) {
      // Create description label and its text field bound to DESCRIPTION
      // controller property
      this.descriptionLabel = new JLabel(SwingTools.getLocalizedLabelText(preferences, HomeFurniturePanel.class,
    ...
    if (controller.isPropertyEditable(HomeFurnitureController.Property.PRICE)) {
      ...
      this.priceLabel = new JLabel(SwingTools.getLocalizedLabelText(preferences, HomeFurniturePanel.class,
          "priceLabel.text"));
      ...
      priceSpinnerModel.setValue(price == null
          ? null
          : price.floatValue());
      final PropertyChangeListener priceChangeListener = new PropertyChangeListener() {
        public void propertyChange(PropertyChangeEvent ev) {
          ...
          priceSpinnerModel.setValue(ev.getNewValue() != null
              ? ((Number)ev.getNewValue()).floatValue()
              : null);
        }
      };
    ...
    if (controller.isPropertyEditable(HomeFurnitureController.Property.ANGLE_IN_DEGREES)
      ...
      angleSpinnerModel.setValue(angle != null
      ...
      final PropertyChangeListener angleChangeListener = new PropertyChangeListener() {
        public void propertyChange(PropertyChangeEvent ev) {
          ...
          angleSpinnerModel.setValue(newAngle != null
              ? new Float((float)Math.toDegrees(newAngle))
              : null);
        }
      };
      ...
      angleSpinnerModel.addChangeListener(new ChangeListener() {
        public void stateChanged(ChangeEvent ev) {
          controller.removePropertyChangeListener(HomeFurnitureController.Property.ANGLE, angleChangeListener);
    ...
    if (!Boolean.getBoolean("com.eteks.sweethome3d.no3D")) {
      if (controller.isPropertyEditable(HomeFurnitureController.Property.ROLL)) {
        // Create roll label and its spinner bound to ROLL_IN_DEGREES controller
        // property
        ...
        rollSpinnerModel.setValue(roll != null
            ? new Float((float)Math.toDegrees(roll))
            : null);
        final PropertyChangeListener rollChangeListener = new PropertyChangeListener() {
          public void propertyChange(PropertyChangeEvent ev) {
            ...
            rollSpinnerModel.setValue(newRoll != null
                ? new Float((float)Math.toDegrees(newRoll))
                : null);
          }
        };
        ...
        rollSpinnerModel.addChangeListener(new ChangeListener() {
          public void stateChanged(ChangeEvent ev) {
            controller.removePropertyChangeListener(HomeFurnitureController.Property.ROLL, rollChangeListener);
      ...
      if (controller.isPropertyEditable(HomeFurnitureController.Property.PITCH)) {
        // Create pitch label and its spinner bound to PITCH_IN_DEGREES
        // controller property
        this.pitchRadioButton = new JRadioButton(SwingTools.getLocalizedLabelText(preferences,
            HomeFurniturePanel.class, "pitchRadioButton.text"));
        ...
        pitchSpinnerModel.setValue(pitch != null
            ? new Float((float)Math.toDegrees(pitch))
            : null);
        final PropertyChangeListener pitchChangeListener = new PropertyChangeListener() {
          public void propertyChange(PropertyChangeEvent ev) {
            ...
            pitchSpinnerModel.setValue(newPitch != null
                ? new Float((float)Math.toDegrees(newPitch))
                : null);
          }
        };
        ...
        pitchSpinnerModel.addChangeListener(new ChangeListener() {
          public void stateChanged(ChangeEvent ev) {
            controller.removePropertyChangeListener(HomeFurnitureController.Property.PITCH, pitchChangeListener);
      ...
      if (this.rollRadioButton != null && this.pitchRadioButton != null) {
        ...
        controller.addPropertyChangeListener(HomeFurnitureController.Property.HORIZONTAL_AXIS,
    ...
    if (controller.isPropertyEditable(HomeFurnitureController.Property.PROPORTIONAL)) {
      ...
      this.keepProportionsCheckBox.setSelected(controller.isProportional()
                                               // Force proportional if
                                               // selection is rotated around
                                               // horizontal axis when no 3D is
                                               // available
                                               || Boolean.getBoolean("com.eteks.sweethome3d.no3D")
                                               && (controller.getRoll() == null || controller.getRoll() != 0
                                                   || controller.getPitch() == null || controller.getPitch() != 0));
    ...
    if (controller.isPropertyEditable(HomeFurnitureController.Property.VISIBLE)) {
      ...
      System.out.println("this.visibleCheckBox.setValue(controller.getVisible());"+(controller.getVisible()));

      final PropertyChangeListener visibleChangeListener = new PropertyChangeListener() {
        public void propertyChange(PropertyChangeEvent ev) {
          System.out.println("homefurniturepanel visibleCheckBox. ev.getNewValue():" + ev.getNewValue());
      ...
      this.visibleCheckBox.addChangeListener(new ChangeListener() {
        public void stateChanged(ChangeEvent ev) {
          System.out.println("homefurniturepanel visibleCheckBox.getValue(): stateChanged" + visibleCheckBox.getValue());
    ...
    if (controller.isPropertyEditable(HomeFurnitureController.Property.SCAN)) {
      // Create visible check box bound to scan controller property
      this.scanCheckBox = new NullableCheckBox(SwingTools.getLocalizedLabelText(preferences, HomeFurniturePanel.class,
          "scanCheckBox.text"));
      this.scanCheckBox.setNullable(controller.getScan() == null);
      this.scanCheckBox.setValue(controller.getScan());
      
      System.out.println("this.scanCheckBox.setVisible(controller.getScan());["+controller.getScan()+"]");
      final PropertyChangeListener scanChangeListener = new PropertyChangeListener() {
        public void propertyChange(PropertyChangeEvent ev) {
          System.out.println("homefurniturepanel scancheckbox. ev.getNewValue():[" + ev.getNewValue()+"]  ev.getPropertyName():["+ev.getPropertyName()+"]");
          scanCheckBox.setNullable(ev.getNewValue() == null);
          scanCheckBox.setValue((Boolean)ev.getNewValue());
        }
      };
      controller.addPropertyChangeListener(HomeFurnitureController.Property.SCAN, scanChangeListener);
      this.scanCheckBox.addChangeListener(new ChangeListener() {
        public void stateChanged(ChangeEvent ev) {
          
          controller.removePropertyChangeListener(HomeFurnitureController.Property.SCAN, scanChangeListener);
          controller.setScan(scanCheckBox.getValue());
          System.out.println("homefurniturepanel scanCheckBox.getValue(): stateChanged" + controller.getScan());
          controller.addPropertyChangeListener(HomeFurnitureController.Property.SCAN, scanChangeListener);
        }
      });
    }
  ...
  private void updateSizeComponents(final HomeFurnitureController controller) {
    ...
    this.keepProportionsCheckBox
        .setEnabled(editableSize
                    && controller.isDeformable()
                    // Disable proportional if selection is rotated around
                    // horizontal axis when no 3D is available
                    && (!Boolean.getBoolean("com.eteks.sweethome3d.no3D") || controller.getRoll() != null
  ...
  private void setMnemonics(UserPreferences preferences) {
    if (!OperatingSystem.isMacOSX()) {
      ...
      if (this.descriptionLabel != null) {
        this.descriptionLabel.setDisplayedMnemonic(KeyStroke.getKeyStroke(
            preferences.getLocalizedString(HomeFurniturePanel.class, "descriptionLabel.mnemonic")).getKeyCode());
  ...
  private void layoutComponents(UserPreferences preferences, final HomeFurnitureController controller) {
    ...
    if (this.nameLabel != null) {
      ...
      namePanel.add(this.nameTextField, new GridBagConstraints(1, 0, 1, 1, 1, 0, GridBagConstraints.LINE_START,
          GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 10), 0, 0));
    }
    if (this.nameVisibleCheckBox != null) {
      namePanel.add(this.nameVisibleCheckBox, new GridBagConstraints(2, 0, 2, 1, 0, 0, GridBagConstraints.LINE_START,
    ...
    if (this.descriptionLabel != null) {
      ...
      namePanel.add(this.descriptionTextField, new GridBagConstraints(1, 1, priceDisplayed
          ? 1
          : 3, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.HORIZONTAL, new Insets(5, 0, 0,
          priceDisplayed
              ? 10
              : 0), 0, 0));
    }
    if (priceDisplayed) {
      namePanel.add(this.priceLabel, new GridBagConstraints(this.descriptionLabel != null
          ? 2
          : 0, 1, 1, 1, 0, 0, labelAlignment, GridBagConstraints.NONE, new Insets(5, 0, 0, 5), 0, 0));
      namePanel.add(this.priceSpinner, new GridBagConstraints(this.descriptionLabel != null
          ? 3
          : 1, 1, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.HORIZONTAL, new Insets(5, 0, 0, 0), 0,
          0));
    }
    if (namePanel.getComponentCount() > 0) {
      add(namePanel, new GridBagConstraints(0, 0, orientationPanelDisplayed
          ? 4
          : 3, 1, 0, 0, labelAlignment, GridBagConstraints.HORIZONTAL, new Insets(0, 0, rowGap, 0), 0, 0));
    }
    // Location panel
    ...
    if (this.angleLabel != null && !orientationPanelDisplayed) {
    ...
    if (locationPanel.getComponentCount() > 0) {
      locationPanel.add(new JLabel(), new GridBagConstraints(0, 100, 2, 1, 0, 1, GridBagConstraints.LINE_START,
          GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
      add(locationPanel, new GridBagConstraints(0, 1, 1, 1, 1, 1, labelAlignment, GridBagConstraints.BOTH, new Insets(
          0, 0, rowGap, 0), 0, 0));
    }
    if (orientationPanelDisplayed) {
      // Orientation panel
      JPanel orientationPanel = SwingTools.createTitledPanel(preferences.getLocalizedString(HomeFurniturePanel.class,
          "orientationPanel.title"));
      JLabel verticalRotationLabel = new JLabel(preferences.getLocalizedString(HomeFurniturePanel.class,
          "verticalRotationLabel.text"));
      JLabel horizontalRotationLabel = new JLabel(preferences.getLocalizedString(HomeFurniturePanel.class,
          "horizontalRotationLabel.text"));
      JLabel orientationLabel = new JLabel(preferences.getLocalizedString(HomeFurniturePanel.class,
          "orientationLabel.text"));
      // There are two possible layout depending whether horizontal and vertical
      // rotation label are defined or not
      ...
      if (this.angleLabel != null) {
        ...
        orientationPanel.add(this.angleLabel,
            new GridBagConstraints(0, 1, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
                new Insets(0, new JRadioButton().getPreferredSize().width, OperatingSystem.isMacOSX()
                                                                           || layoutWithHorizontalVerticalLabels
                    ? 5
                    : 2, 5), 0, 0));
        orientationPanel.add(this.angleSpinner, new GridBagConstraints(1, 1, 1, 1, 0, 0, GridBagConstraints.LINE_START,
            GridBagConstraints.HORIZONTAL, rightComponentInsets, 10, 0));
      }
      Insets secondRadioButtonInsets = new Insets(0, 0, layoutWithHorizontalVerticalLabels
          ? 2
          : (OperatingSystem.isMacOSX()
              ? 5
              : 1), 5);
      Insets secondSpinnerInsets = new Insets(0, 0, layoutWithHorizontalVerticalLabels
          ? 2
          : 5, 0);
      ...
      if (this.pitchRadioButton != null) {
        ...
        orientationPanel.add(this.pitchRadioButton, new GridBagConstraints(0, layoutWithHorizontalVerticalLabels
            ? 3
            : 4, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE, layoutWithHorizontalVerticalLabels
            ? secondRadioButtonInsets
            : thirdRadioButtonInsets, 0, 0));
        orientationPanel.add(this.pitchSpinner, new GridBagConstraints(1, layoutWithHorizontalVerticalLabels
            ? 3
            : 4, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.HORIZONTAL,
            layoutWithHorizontalVerticalLabels
                ? secondSpinnerInsets
                : thirdSpinnerinsets, 10, 0));
      }
      if (this.rollRadioButton != null) {
        orientationPanel.add(this.rollRadioButton, new GridBagConstraints(0, layoutWithHorizontalVerticalLabels
            ? 4
            : 3, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE, layoutWithHorizontalVerticalLabels
            ? thirdRadioButtonInsets
            : secondRadioButtonInsets, 0, 0));
        orientationPanel.add(this.rollSpinner, new GridBagConstraints(1, layoutWithHorizontalVerticalLabels
            ? 4
            : 3, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.HORIZONTAL,
            layoutWithHorizontalVerticalLabels
                ? thirdSpinnerinsets
                : secondSpinnerInsets, 10, 0));
      }
      if (this.rollRadioButton != null && this.pitchRadioButton != null) {
        if (controller.isTexturable()) {
          // Do not display orientation label information for not texturable
          // furniture to keep a balanced dialog box
          orientationPanel.add(new JLabel(new ImageIcon(getClass().getResource("resources/furnitureOrientation.png"))),
              new GridBagConstraints(0, 6, 2, 1, 1, layoutWithHorizontalVerticalLabels
                  ? 1
                  : 0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(10, 0, 5, 0), 0, 0));
        }
        if (layoutWithHorizontalVerticalLabels) {
          // Add information labels about axes adjusting insets to align with
          // components in other panels
          orientationPanel.add(verticalRotationLabel, new GridBagConstraints(0, 0, 2, 1, 0, 0,
              GridBagConstraints.LINE_START, GridBagConstraints.NONE, new Insets(OperatingSystem.isMacOSX()
                  ? 5
                  : 3, 0, OperatingSystem.isMacOSX()
                  ? 10
                  : 8, 0), 0, 0));
          orientationPanel.add(horizontalRotationLabel, new GridBagConstraints(0, 2, 2, 1, 0, 0,
              GridBagConstraints.LINE_START, GridBagConstraints.NONE, new Insets(OperatingSystem.isMacOSX()
                  ? 5
                  : 3, 0, OperatingSystem.isMacOSX()
                  ? 9
          ...
          orientationPanel.add(orientationLabel, new GridBagConstraints(0, 7, 2, 1, 1, 1, GridBagConstraints.NORTH,
              GridBagConstraints.NONE, new Insets(10, 0, 0, 0), 0, 0));
        }
      }
      if (orientationPanel.getComponentCount() > 0) {
        add(orientationPanel, new GridBagConstraints(1, 1, 1, 2, 1, 0, labelAlignment, GridBagConstraints.BOTH,
    ...
    if (this.keepProportionsCheckBox != null) {
      sizePanel.add(this.keepProportionsCheckBox, new GridBagConstraints(0, 3, 2, 1, 0, 0,
          GridBagConstraints.LINE_START, GridBagConstraints.NONE, new Insets(0, 0, OperatingSystem.isMacOSX()
              ? 5
              : 2, 0), 0, 0));
    }
    if (this.mirroredModelCheckBox != null) {
      sizePanel.add(this.mirroredModelCheckBox, new GridBagConstraints(0, 4, 2, 1, 0, 1, GridBagConstraints.LINE_START,
          GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
    }
    if (sizePanel.getComponentCount() > 0) {
      add(sizePanel, new GridBagConstraints(orientationPanelDisplayed
          ? 2
          : 1, 1, 2, 1, 1, 1, labelAlignment, GridBagConstraints.BOTH, new Insets(0, 0, rowGap, 0), 0, 0));
    }
    final JPanel paintPanel = SwingTools.createTitledPanel(preferences.getLocalizedString(HomeFurniturePanel.class,
        "colorAndTexturePanel.title"));
    if (this.defaultColorAndTextureRadioButton != null) {
      int buttonPadY;
      ...
      paintPanel.add(this.colorRadioButton, new GridBagConstraints(0, 1, 1, 1, 0, 0, GridBagConstraints.LINE_START,
      ...
      if (this.textureComponent != null) {
        paintPanel.add(this.textureRadioButton, new GridBagConstraints(0, 2, 1, 1, 0, 0, GridBagConstraints.LINE_START,
            GridBagConstraints.NONE, new Insets(5, 0, 0, 5), 0, 0));
        paintPanel.add(this.textureComponent, new GridBagConstraints(1, 2, 1, 1, 0, 0, GridBagConstraints.LINE_START,
      ...
      if (this.modelMaterialsComponent != null) {
        ...
        paintPanel.add(this.modelMaterialsComponent, new GridBagConstraints(1, 3, 1, 1, 0, 0,
            GridBagConstraints.LINE_START, GridBagConstraints.HORIZONTAL, new Insets(5, 0, buttonsBottomInset, 0), 0,
            buttonPadY));
      }
      add(paintPanel, new GridBagConstraints(0, 2, 1, 1, 0, 0, labelAlignment, GridBagConstraints.BOTH, new Insets(0,
          0, rowGap, 0), 0, 0));
    ...
    if (this.defaultShininessRadioButton != null) {
      ...
      add(shininessPanel, new GridBagConstraints(orientationPanelDisplayed
          ? 2
          : 1, 2, 2, 1, 0, 0, labelAlignment, GridBagConstraints.BOTH, new Insets(0, 0, rowGap, 0), 0, 0));
    ...
    // Last row
    if (this.scanCheckBox != null) {
      add(this.scanCheckBox, new GridBagConstraints(0, 3, 1, 1, 0, 0, GridBagConstraints.LINE_START,
          GridBagConstraints.NONE, new Insets(0, 100, 0, 0), 0, 0));
    }
    if (this.lightPowerLabel != null) {
      add(this.lightPowerLabel, new GridBagConstraints(orientationPanelDisplayed
          ? 2
          : 1, 3, 1, 1, 0, 0, labelAlignment, GridBagConstraints.NONE, new Insets(0, 10, 0, 5), 0, 0));
      add(this.lightPowerSpinner, new GridBagConstraints(orientationPanelDisplayed
          ? 3
          : 2, 3, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE, new Insets(0, 0, 0, 5), 0, 0));
    }
  }
  ...
  public void displayView(View parentView) {
    if (SwingTools.showConfirmDialog((JComponent)parentView, this, this.dialogTitle, this.nameTextField) == JOptionPane.OK_OPTION) {
      System.out.println("HomeFurniturePanel ok_option click  this.controller.modifyFurniture is called");
