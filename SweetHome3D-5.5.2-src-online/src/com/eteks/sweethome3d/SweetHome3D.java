...
import com.eteks.sweethome3d.swing.CasePanel;
 ...
 * Users may also define the optional following System properties to alter
 * program behavior:
 * 
 * <ul>
 * <li><code>com.eteks.sweethome3d.applicationFolders</code> defines the
 * folder(s) where private files of Sweet Home 3D are stored. Private files
 * include SH3F furniture library files stored in a subfolder named
 * <code>furniture</code>, SH3T textures library files stored in a subfolder
 * named <code>textures</code>, SH3L language library files stored in a
 * subfolder named <code>languages</code>, SH3P application plug-in files stored
 * in a subfolder named <code>plugins</code> and SH3D files automatically
 * created for recovery purpose stored in a subfolder named
 * <code>recovery</code>.<br>
 * If this property describes more than one folder, they should be separated by
 * a semicolon (;) under Windows or by a colon (:) under Mac OS X and Unix
 * systems. The first folder listed in this property is used as the folder where
 * will be stored recovered SH3D files and SH3F, SH3T, SH3L, SH3P files imported
 * by the user. Thus the user should have write access rights on this first
 * folder otherwise he won't be able to import SH3F, SH3T, SH3L or SH3P files,
 * and auto recovered SH3D files won't be managed. If this folder or one of its
 * <code>furniture</code>, <code>textures</code>, <code>languages</code>,
 * <code>plugins</code>, <code>recovery</code> subfolders don't exist, Sweet
 * Home 3D will create it when needed.<br>
 * The other folders are used as resources where SH3F, SH3T, SH3L, SH3P files
 * will be searched in their respective <code>furniture</code>,
 * <code>textures</code>, <code>languages</code>, <code>plugins</code>
 * subfolders. Any of the cited folders may be an absolute path or a relative
 * path to the folder from which the program was launched.</li>
 * 
 * <li><code>com.eteks.sweethome3d.preferencesFolder</code> defines the folder
 * where preferences files (<code>preferences.xml</code> and the files depending
 * on it) are stored. The user should have write access rights on this folder
 * otherwise the program won't be able to save his preferences and the files he
 * imported in furniture and textures catalogs. This folder may be the same as
 * the folder cited in <code>com.eteks.sweethome3d.applicationFolders</code>
 * property.</li>
 * 
 * <li><code>com.eteks.sweethome3d.no3D</code> should be set to
 * <code>true</code> if 3D capabilities (including 3D view and importing
 * furniture 3D models) shouldn't be used in Sweet Home 3D.
 * 
 * <li><code>com.eteks.sweethome3d.j3d.checkOffScreenSupport</code> should be
 * set to <code>false</code> when editing preferences, printing, creating a
 * photo or creating a video always lead to a crash of Sweet Home 3D. This means
 * offscreen 3D images isn't supported by your video driver and Sweet Home 3D
 * doesn't even succeed to test this support. Setting this System property to
 * <code>false</code> disables this test.</li>
 * 
 * <li><code>com.eteks.sweethome3d.j3d.additionalLoaderClasses</code> defines
 * additional Java 3D {@linkplain com.sun.j3d.loaders.Loader loader} classes
 * that Sweet Home 3D will use to read 3D models content at formats not
 * supported by default in Sweet Home 3D.<br>
 * The classes cited in this property must be available in the classpath and if
 * more than one class is cited, they should be separated by a colon or a space.
 * </li>
 * </ul>
 * 
 * <p>
 * The value of a System property can be set with the -D <a href=
...
public class SweetHome3D extends HomeApplication {
  ...
  public UserPreferences getUserPreferences() {
    ...
    if (this.userPreferences == null) {
      ...
      System.out.println("applicationFoldersProperty" + applicationFoldersProperty + " preferencesFolderProperty:"
      ...
      System.out.println(String.format("preferencesFolder[ %s], applicationFolders[ %s]", preferencesFolder,
          applicationFolders));
      this.userPreferences = new FileUserPreferences(preferencesFolder, applicationFolders, eventQueueExecutor) {
        @Override
        public List<Library> getLibraries() {
          if (userPreferences != null // Don't go further if preferences are not
                                      // ready
  ...
  JFrame getHomeFrame(Home home) {
    System.out.println("(JFrame)SwingUtilities.getRoot((JComponent)this.homeFrameControllers.get(home).getView())"
                       + home);
    JFrame frame = (JFrame)SwingUtilities.getRoot((JComponent)this.homeFrameControllers.get(home).getView());
  ...
  private void showHomeFrame(Home home) {
    ...
    // System.out.println("homeFrame.getContentPane().setBackground(Color.YELLOW);");
    // homeFrame.getContentPane().setBackground(Color.YELLOW);
    homeFrame.toFront();
  }

  /**
   * Sweet Home 3D entry point.
   * @param args may contain one .sh3d, .sh3x, .sh3f, .sh3t, .sh3l or .sh3p file
   *          to open or install, following a <code>-open</code> option.
   */
  ...
  protected void init(final String [] args) {
    ...
    addHomesListener(new CollectionListener<Home>() {
      ...
      public void collectionChanged(CollectionEvent<Home> ev) {
        switch (ev.getType()) {
          case ADD:
            System.out.println("addHomesListener(new CollectionListener<Home>() {  start");
            Home home = ev.getItem();
            
            try {
              System.out.println("HomeFrameController controller = createHomeFrameController(home); start");
              HomeFrameController controller = createHomeFrameController(home);
              System.out.println("HomeFrameController controller = createHomeFrameController(home); end");
              controller.displayView();
              System.out.println("controller.displayView(); end");
              ...
              // Check exception by class name to avoid a mandatory bind to Java
              // 3D
            ...
            System.out.println("addHomesListener(new CollectionListener<Home>() {  end");
            break;
          case DELETE:
            ...
            if (getHomes().isEmpty()
                && (!OperatingSystem.isMacOSX() || !Boolean.getBoolean("apple.laf.useScreenMenuBar") || OperatingSystem
                    .isJavaVersionGreaterOrEqual("1.9"))) {
              ...
              // Exit once current events are managed (under Mac OS X, exit is
              // managed by MacOSXConfiguration)
              EventQueue.invokeLater(new Runnable() {
                public void run() {
                  System.exit(0);
                }
...
//    EventQueue.invokeLater(new Runnable() {
//      public void run() {
//        System.out.println("EventQueue.invokeLater(new Runnable() SweetHome3D.this.start(args);");
//        SweetHome3D.this.start(args);
//        System.out.println("EventQueue.invokeLater(new Runnable() SweetHome3D.this.start(args);end");
//      }
//    });
    
    CasePanel casePanel = new CasePanel(this.getUserPreferences(), this, null);
    casePanel.displayView(null);
  }
  ...
  private void initSystemProperties() {
    if (OperatingSystem.isMacOSX()) {
      ...
      System.out.println("applicationName:" + applicationName + "  classPackage:" + classPackage + "." + "package");
  ...
  public static void setBkColor(Container c, Color clr) {
    ...
    System.out.println("setBkColorsetBkColor "+c+"  color"+clr);
  ...
  private void initLookAndFeel() {
    try {
      ...
      UIManager.put("ToolBar.background", new Color(33, 33, 33));
//      UIManager.put("ToolBar.foreground", new Color(33, 33, 33));
      UIManager.put("ToolBar.opaque", true);
      UIManager.put("control", new Color(128, 128, 128));
      UIManager.put("info", new Color(128, 128, 128));
//    UIManager.put("nimbusBase", new Color(18, 30, 49));
      UIManager.put("nimbusBase", new Color(33, 33, 33));
...
//      UIManager
//          .setLookAndFeel(System.getProperty("swing.defaultlaf", UIManager.getCrossPlatformLookAndFeelClassName()));
      
      // Change default titled borders under Mac OS X 10.5
      if (OperatingSystem.isMacOSXLeopardOrSuperior()) {
        UIManager.put("TitledBorder.border", UIManager.getBorder("TitledBorder.aquaVariant"));
      }
      for(Enumeration<Object> i= UIManager.getDefaults().keys();i.hasMoreElements();){
        Object key=i.nextElement();
        System.out.println("Object key=i.nextElement  "+key+":"+UIManager.get(key));
      }
      if (OperatingSystem.isMacOSXYosemiteOrSuperior()) {
        UIManager.put("SplitPaneDivider.border", new Border() {
          public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
            // Set a border that fills the divider with the expected background
            ...
            if (splitPane.getOrientation() == JSplitPane.VERTICAL_SPLIT) {
              clipArea.subtract(new Area(new Polygon(new int [] {x, x + 4, x + 8}, new int [] {y + height / 2 + 3,
                y + height / 2 - 3, y + height / 2 + 3}, 3)));
              clipArea.subtract(new Area(new Polygon(new int [] {x + 12, x + 15, x + 19}, new int [] {
                y + height / 2 - 2, y + height / 2 + 3, y + height / 2 - 2}, 3)));
            }
            g.setClip(clipArea);
        ...
        public void collectionChanged(CollectionEvent<Home> ev) {
          ...
          if (ev.getType() == CollectionEvent.Type.ADD) {
            if (ev.getItem().getName() != null && home.getName() == null && !home.isRecovered()) {
              if (OperatingSystem.isMacOSXLionOrSuperior() && OperatingSystem.isJavaVersionGreaterOrEqual("1.7")
  ...
  public void start(String [] args) {
    System.out.println("sweethome start is invoked args");
    for (String a : args) {
    ...
    if (args.length == 2 && args [0].equals("-open") && args [1].length() > 0) {
      ...
      if (getContentManager().isAcceptable(args [1], ContentManager.ContentType.SWEET_HOME_3D)) {
        // Add a listener to application to recover homes once the one in
        // parameter is open
        addHomesListener(new CollectionListener<Home>() {
          public void collectionChanged(CollectionEvent<Home> ev) {
            if (ev.getType() == CollectionEvent.Type.ADD) {
      ...
      System.out.println("showDefaultHomeFrame();");
      showDefaultHomeFrame();
      System.out.println("showDefaultHomeFrame();end");
//      checkUpdates();
    }
  }

  /**
   * Shows a home frame, either a new one when no home is opened, or the last
   * created home frame.
   */
  private void showDefaultHomeFrame() {
    System.out.println("getHomes().isEmpty()" + getHomes().isEmpty() + "this.autoRecoveryManager is "
                       + (this.autoRecoveryManager));
    // getHomes().isEmpty()truethis.autoRecoveryManager is
    // Nullcom.eteks.sweethome3d.io.AutoRecoveryManager@79aba111
    if (getHomes().isEmpty()) {
      if (this.autoRecoveryManager != null) {
        System.out.println(" his.autoRecoveryManager.openRecoveredHomes();");
        this.autoRecoveryManager.openRecoveredHomes();
        System.out.println(" his.autoRecoveryManager.openRecoveredHomes(); end");
      }
      System.out.println("getHomes().isEmpty()");
      if (getHomes().isEmpty()) {
        // Add a new home to application
        System.out.println(" // Add a new home to application");
        addHome(createHome());
      }
      System.out.println("getHomes().isEmpty() end");
    } else {
  ...
  private static class FileContentManagerWithRecordedLastDirectories extends FileContentManager {
    ...
    protected void setLastDirectory(ContentType contentType, File directory) {
      // Last directories are not recorded in user preferences since there's no
      // need of portability
      ...
      try {
        ...
        // Ignore exception, Sweet Home 3D will work without recorded
        // directories
      }
    }
  }
    ...
    public void addSingleInstanceListener(SingleInstanceListener l) {
      if (this.singleInstanceListeners.isEmpty()) {
        if (!OperatingSystem.isMacOSX()) {
          ...
          // launches will be notified
          // by com.apple.eawt.ApplicationListener added to application in
          // MacOSXConfiguration class
