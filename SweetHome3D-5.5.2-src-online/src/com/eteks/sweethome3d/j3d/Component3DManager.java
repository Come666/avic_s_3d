 ...
 * Note: this class is compatible with Java 3D 1.3 at runtime but requires Java
 * 3D 1.5 to compile.
...
public class Component3DManager {
  ...
  // (use Object class to ensure Component3DManager class can run with Java 3D
  // 1.3.1)
  ...
  /**
   * Sets the current rendering error listener bound to
  ...
  public void setRenderingErrorObserver(RenderingErrorObserver observer) {
    try {
      ...
      // As RenderingErrorListener and addRenderingErrorListener are available
      // since Java 3D 1.5,
      // use the default rendering error reporting if Sweet Home 3D is linked to
      // a previous version
    }
  }

  /**
   * Returns the current rendering error listener bound to
   * <code>VirtualUniverse</code>.
   */
  ...
  /**
   * Returns <code>true</code> if offscreen is supported in Java 3D on user
   * system. Will always return <code>false</code> if
   * <code>com.eteks.sweethome3d.j3d.checkOffScreenSupport</code> system is
   * equal to <code>false</code>. By default,
   * <code>com.eteks.sweethome3d.j3d.checkOffScreenSupport</code> is equal to
   * <code>true</code>.
   */
    ...
    if (this.offScreenImageSupported == null) {
      if ("false".equalsIgnoreCase(System.getProperty(CHECK_OFF_SCREEN_IMAGE_SUPPORT, "true"))) {
        ...
        try {
          ...
          // Create a dummy 3D image to check if it can be rendered in current
  ...
  /**
   * Returns a new <code>canva3D</code> instance that will call
   * <code>renderingObserver</code> methods during the rendering loop.
   * @throws IllegalRenderingStateException if the canvas 3D couldn't be
   *           created.
   */
  private Canvas3D getCanvas3D(GraphicsConfiguration deviceConfiguration, boolean offscreen,
    ...
    try {
      ...
      if (!offscreen && OperatingSystem.isLinux() && OperatingSystem.isJavaVersionGreaterOrEqual("1.7")) {
        // Add a listener to the parent window once known that will repaint the
        // canvas in 100 ms
        // when the window is activated / deactivated to avoid keeping an empty
        // canvas
  ...
  /**
   * Returns a new on screen <code>canva3D</code> instance. The returned canvas
   * 3D will be associated with the graphics configuration of the default screen
   * device.
   * @throws IllegalRenderingStateException if the canvas 3D couldn't be
   *           created.
   */
  ...
  /**
   * Returns a new on screen <code>canva3D</code> instance which rendering will
   * be observed with the given rendering observer. The returned canvas 3D will
   * be associated with the graphics configuration of the default screen device.
   * @param renderingObserver an observer of the 3D rendering process of the
   *          returned canvas. Caution: The methods of the observer will be
   *          called in 3D rendering loop thread.
   * @throws IllegalRenderingStateException if the canvas 3D couldn't be
   *           created.
   */
  public Canvas3D getOnscreenCanvas3D(RenderingObserver renderingObserver) {
    System.out.println("Canvas3D getOnscreenCanvas3D(RenderingObserver renderingObserver)");
    return getCanvas3D(null, false, renderingObserver);
  }

  /**
   * Returns a new on screen <code>canva3D</code> instance which rendering will
   * be observed with the given rendering observer.
   * @param renderingObserver an observer of the 3D rendering process of the
   *          returned canvas. Caution: The methods of the observer will be
   *          called in 3D rendering loop thread.
   * @throws IllegalRenderingStateException if the canvas 3D couldn't be
  ...
  public Canvas3D getOnscreenCanvas3D(GraphicsConfiguration deviceConfiguration, RenderingObserver renderingObserver) {
    Canvas3D canvas3d = getCanvas3D(deviceConfiguration, false, renderingObserver);
    System.out
        .println("Canvas3D getOnscreenCanvas3D(GraphicsConfiguration deviceConfiguration,  RenderingObserver renderingObserver) {  "
                 + canvas3d);
    return canvas3d;
  }
   ...
   * @throws IllegalRenderingStateException if the canvas 3D couldn't be
   *           created. To avoid this exception, call
   *           {@link #isOffScreenImageSupported() isOffScreenImageSupported()}
   *           first.
   */
  public Canvas3D getOffScreenCanvas3D(int width, int height) {

    System.out.println("getOffScreenCanvas3D(int width, int height)");
  ...
  /**
   * Returns an image at the given size of the 3D <code>view</code>. This image
   * is created with an off screen canvas.
  ...
  public BufferedImage getOffScreenImage(View view, int width, int height) {
    ...
    try {
      ...
      // a latch to check further if a rendering error happened during off
      // screen rendering
      ...
      // If latch count becomes equal to 0 during the past instructions or in
      // the coming 10 milliseconds,
  ...
  /**
   * Manages Java 3D 1.5 <code>RenderingErrorListener</code> change matching the
   * given rendering error observer.
   */
  ...
  /**
   * An observer that receives notifications during the different steps of the
   * loop rendering a canvas 3D.
   */
  ...
  private static class ObservedCanvas3D extends Canvas3D {
    ...
    private ObservedCanvas3D(GraphicsConfiguration graphicsConfiguration, boolean offScreen,
      ...
      // Under Windows with Java 7 and above, delay the rendering of the canvas
      // 3D when
      // it's repainted (i.e. it's resized, moved or partially hidden) to avoid
      // it to get grayed
