...
public class MyMousePickBehavior extends PickMouseBehavior {
  ...
  public MyMousePickBehavior(Canvas3D canvas, BranchGroup root, Bounds bounds, Selectable homeObject) {
    ...
    this.setEnable(true);
    root.addChild(this);
    this.setMode(PickTool.GEOMETRY_INTERSECT_INFO);
    System.out.println("MyMousePickBehavior constract is invorked"+jta);
  ...
  // if (state == 0) {
  // data [getXintValue(x)] [getYintValue(y)] = data [getXintValue(x)]
  // [getYintValue(y)] | getZintValue(z);
  ...
  public void updateScene(int xpos, int ypos) {
    ...
    pickCanvas.setShapeLocation(xpos, ypos);
    pickResult = pickCanvas.pickClosest();
    System.out.println("updateScene(int xpos, int ypos):" + xpos + " " + ypos + " pickResult:" + pickResult);
    if (pickResult != null) {
      shape = (Sphere)pickResult.getNode(PickResult.PRIMITIVE);
      Node tmpShape = pickResult.getNode(PickResult.SHAPE3D);
      System.out.println("tmpShape :" + tmpShape);
      if (tmpShape != null) {
        System.out.println(tmpShape.getName());
      }
    }
  }
}
