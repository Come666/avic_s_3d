...
public class LabelController implements Controller {
  ...
  public enum Property {
    TEXT, FONT_NAME, FONT_SIZE, COLOR, PITCH, ELEVATION, IMAGENAME
  }
  ...
  public LabelController(Home home, UserPreferences preferences, ViewFactory viewFactory,
    ...
    this.contentManager = new FileContentManager(preferences);
    updateProperties();
  }
  ...
  public LabelController(Home home, float x, float y, UserPreferences preferences, ViewFactory viewFactory,
    ...
    this.fontSize = preferences.getDefaultTextStyle(LabelImage.class).getFontSize();
  ...
  public void setImageName(String imageName) {
    System.out.println("setImageName(String imageName)" + imageName);
    if (imageName != this.imageName) {
      String oldImageName = this.imageName;
      this.imageName = imageName;
      this.propertyChangeSupport.firePropertyChange(Property.IMAGENAME.name(), oldImageName, imageName);
    }
  }

  /**
   * Updates edited properties from selected labels in the home edited by this
   * controller.
   */
  protected void updateProperties() {
    List<LabelImage> selectedLabels = Home.getLabelsSubList(this.home.getSelectedItems());
    if (selectedLabels.isEmpty()) {
      ...
      LabelImage firstLabel = selectedLabels.get(0);
      ...
      System.out.println("firstLabel.getText():"+text);
      System.out.println("this.getText():"+this.getText());
      if(  this.getText()==null || this.getText().equalsIgnoreCase(text) ){
        setText(text);
      }
      System.out.println("firstLabel.getImageName():" + firstLabel.getImageName());
//      setImageName(this.getImageName());
       if (this.imageName != null &&
       !this.imageName.equalsIgnoreCase(firstLabel.getImageName())) {
         firstLabel.setImageName(this.imageName);
       }
      ...
      for (int i = 1; i < selectedLabels.size(); i++) {
        LabelImage label = selectedLabels.get(i);
        if (!(fontName == null && (label.getStyle() == null || label.getStyle().getFontName() == null) || fontName != null
                                                                                                          && label
                                                                                                              .getStyle() != null
                                                                                                          && fontName
                                                                                                              .equals(label
      ...
      float labelDefaultFontSize = this.preferences.getDefaultTextStyle(LabelImage.class).getFontSize();
  ...
  /**
   * Returns <code>Boolean.TRUE</code> if all edited labels are viewed in 3D, or
   * <code>Boolean.FALSE</code> if no label is viewed in 3D.
   */
  ...
  public void createLabel() {
    ...
    if (text != null && text.trim().length() > 0) {
      ...
      LabelImage label = new LabelImage(text, x, y, imageName);
      String fontName = getFontName();
      Float fontSize = getFontSize();
      if (fontName != null || fontSize != null || getPitch() != null) {
        TextStyle style = this.preferences.getDefaultTextStyle(LabelImage.class);
        if (fontName != null) {
  ...
  /**
   * Undoable edit for label creation. This class isn't anonymous to avoid being
   * bound to controller and its view.
   */
  private static class LabelCreationUndoableEdit extends AbstractUndoableEdit {
    ...
    private final LabelImage       label;
    ...
    private LabelCreationUndoableEdit(Home home, UserPreferences preferences, List<Selectable> oldSelection,
                                      boolean oldBasePlanLocked, boolean oldAllLevelsSelection, LabelImage label,
   ...
   */
  private static void doAddLabel(Home home, LabelImage label, boolean basePlanLocked) {
    home.addLabel(label);
   ...
   */
  private static void doDeleteLabel(Home home, LabelImage label, boolean basePlanLocked) {
   ...
   */
  protected boolean isLabelPartOfBasePlan(LabelImage label) {
    return true;
  }
  ...
  public void modifyLabels() {
    List<Selectable> oldSelection = this.home.getSelectedItems();
    List<LabelImage> selectedLabels = Home.getLabelsSubList(oldSelection);
    if (!selectedLabels.isEmpty()) {
      String text = getText();
      String fontName = getFontName();
      ...
      // Apply modification
      TextStyle defaultStyle = this.preferences.getDefaultTextStyle(LabelImage.class);
      doModifyLabels(modifiedLabels, text, fontName, fontNameSet, fontSize, defaultStyle, color, pitch, pitchEnabled,
          elevation,imageName);
      if (this.undoSupport != null) {
        UndoableEdit undoableEdit = new LabelModificationUndoableEdit(this.home, this.preferences, oldSelection,
            modifiedLabels, text, fontName, fontNameSet, fontSize, defaultStyle, color, pitch, pitchEnabled, elevation, imageName);
        this.undoSupport.postEdit(undoableEdit);
      }
      this.preferences.addAutoCompletionString("LabelText", text);
    }
  ...
  private static class LabelModificationUndoableEdit extends AbstractUndoableEdit {
    ...
    private LabelModificationUndoableEdit(Home home, UserPreferences preferences, List<Selectable> oldSelection,
                                          ModifiedLabel [] modifiedLabels, String text, String fontName,
                                          boolean fontNameSet, Float fontSize, TextStyle defaultStyle, Integer color,
                                          Float pitch, Boolean pitchEnabled, Float elevation, String imageName) {
      this.home = home;
      this.preferences = preferences;
      this.oldSelection = oldSelection;
    ...
    public void redo() throws CannotRedoException {
      ...
      doModifyLabels(this.modifiedLabels, this.text, this.fontName, this.fontNameSet, this.fontSize, this.defaultStyle,
          this.color, this.pitch, this.pitchEnabled, this.elevation, this.imageName);
      this.home.setSelectedItems(this.oldSelection);
    }
  ...
  private static void doModifyLabels(ModifiedLabel [] modifiedLabels, String text, String fontName,
    ...
    for (ModifiedLabel modifiedLabel : modifiedLabels) {
      LabelImage label = modifiedLabel.getLabel();
      if (text != null) {
        label.setText(text);
      }
      if (fontNameSet) {
      ...
      if(imageName!=null){
        label.setImageName(imageName);
      }
    }
  }

  /**
   * Restores label properties from the values stored in
   * <code>modifiedLabels</code>.
   */
  ...
  private static final class ModifiedLabel {
    ...
    public ModifiedLabel(LabelImage label) {
      this.label = label;
