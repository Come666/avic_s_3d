...
/**
 * A pane that displays a {@link com.eteks.sweethome3d.swing.HomePane home pane}
 * in a frame.
...
public class HomeFramePane extends JRootPane implements View {
  ...
  public void displayView() {
    ...
    System.out.println("OperatingSystem.isMacOSXLeopardOrSuperior():" + OperatingSystem.isMacOSXLeopardOrSuperior());
    ...
    HomeView homeView = this.controller.getHomeController().getView();
    System.out.println("homeView instanceof JRootPane:" + (homeView instanceof JRootPane));
    ...
    homeFrame.getContentPane().setBackground(Color.red);
    Dimension dimension=Toolkit.getDefaultToolkit().getScreenSize();
    homeFrame.setVisible(true);
    homeFrame.setSize(dimension);
    
    
    EventQueue.invokeLater(new Runnable() {
      public void run() {
        // Add state listener to frame once it's visible to avoid some undesired
        // events during first showing
        addWindowStateListener(home, application, controller.getHomeController(), homeFrame);
        // Request the frame to go to front again because closing waiting dialog
        // meanwhile
  ...
  private void addListeners(final Home home, final HomeApplication application, final HomeController controller,
    ...
    final WindowAdapter windowListener = new WindowAdapter() {
      ...
      public void windowDeactivated(WindowEvent ev) {
        // Java 3D 1.5 bug : windowDeactivated notifications should not be sent
        // to this frame
        // while canvases 3D are created in a child modal dialog like the one
        // managing
        // ImportedFurnitureWizardStepsPanel. As this makes Swing loose the most
        // recent focus owner
        // let's store it in a field to use it when this frame will be
        // reactivated.
      ...
      public void windowActivated(WindowEvent ev) {
        // Java 3D 1.5 bug : let's request focus in window for the most recent
        // focus owner when
    ...
    // Add a listener to preferences to apply component orientation to frame
   ...
   */
  private void addWindowStateListener(final Home home, final HomeApplication application,
                                      final HomeController controller, final JFrame frame) {
    // Control frame closing and activation
    ...
    if (x != null && y != null && width != null && height != null && screenWidth != null && screenHeight != null
      ...
      if (maximized) {
        if (OperatingSystem.isMacOSX() && OperatingSystem.isJavaVersionGreaterOrEqual("1.7")) {
          // Display the frame at its maximum size because calling
          // setExtendedState to maximize
        ...
        // Add a listener that will set the normal size when the frame leaves
        // the maximized state
        WindowAdapter windowStateListener = new WindowAdapter() {
          @Override
          public void windowStateChanged(WindowEvent ev) {
            if ((ev.getOldState() == JFrame.MAXIMIZED_BOTH || (OperatingSystem.isMacOSX()
                                                               && OperatingSystem.isJavaVersionGreaterOrEqual("1.7") && ev
                .getOldState() == JFrame.NORMAL)) && ev.getNewState() == JFrame.NORMAL) {
              if (OperatingSystem.isMacOSXLionOrSuperior()) {
                // Set back frame size later once frame reduce animation is
                // finished
  ...
  private void updateFrameTitle(JFrame frame, Home home, HomeApplication application) {
    ...
    System.out.println("updateFrameTitle:String homeName = home.getName:" + homeName);
    ...
    System.out.println("String title = homeDisplayedName" + title);
    if (OperatingSystem.isMacOSX()) {
      ...
      if (!frame.isVisible() && OperatingSystem.isMacOSXLionOrSuperior()) {
        try {
          // Call Mac OS X specific
          // FullScreenUtilities.setWindowCanFullScreen(homeFrame, true) by
          // reflection
