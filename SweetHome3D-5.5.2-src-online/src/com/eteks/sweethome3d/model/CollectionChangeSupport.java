...
public class CollectionChangeSupport<T> {
  ...
  public void fireCollectionChanged(T item, int index, 
    ...
    System.out.println("!this.collectionListeners.isEmpty()"+(!this.collectionListeners.isEmpty()));
    if (!this.collectionListeners.isEmpty()) {
      System.out.println("CollectionEvent<T> event = new CollectionEvent<T>(this.source, item, index, eventType); start");

      CollectionEvent<T> event = new CollectionEvent<T>(this.source, item, index, eventType);
      System.out.println("CollectionEvent<T> event = new CollectionEvent<T>(this.source, item, index, eventType); end");
      // Work on a copy of collectionListeners to ensure a listener 
      // can modify safely listeners list
      System.out.println("CollectionEvent<T> event = new CollectionEvent<T>(this.source, item, index, eventType);"+event+"   source:"+this.source);
